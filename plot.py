...
# Plot bars
ax.bar(x - width, best_known, width, label='Best Known', 
       color=best_color, edgecolor='white', linewidth=2, alpha=0.85)

ax.bar(x, cp_bars, width, label='CP-SAT', 
       color=cp_color, edgecolor='white', linewidth=2, alpha=0.85)

ax.bar(x + width, dqn_bars, width, label='LSTM-A', 
       color=dqn_color, edgecolor='white', linewidth=2, alpha=0.85)

# Gray placeholder for missing CP-SAT
ax.bar(8, -1, width, color=missing_color, 
       edgecolor='#CCCCCC', linewidth=1.5, alpha=0.6)
...
# ==================== SAVE ====================
plt.savefig('fig_energy_comparison.png', dpi=600, bbox_inches='tight', facecolor='white')
